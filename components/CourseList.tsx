...
const CourseCard: React.FC<{ course: Course; onCardClick: () => void; onUnlockClick: () => void }> = ({ course, onCardClick, onUnlockClick }) => (
  <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col group transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
    ...
    <div className="p-5 flex flex-col flex-grow">
      ...
      {course.isFree ? (
        <button onClick={onCardClick} className="mt-auto w-full bg-brand-primary text-white font-bold py-2 px-4 rounded-lg hover:bg-brand-secondary transition-all duration-300 transform hover:scale-105 active:scale-95 shadow-md hover:shadow-brand-primary/40">
          Start Learning
        </button>
      ) : (
        <button onClick={onUnlockClick} className="mt-auto w-full bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors duration-300 flex items-center justify-center transform hover:scale-105 active:scale-95">
...
const CourseList: React.FC<CourseListProps> = ({ courses, navigateToCourse, initialCategory = 'All' }) => {
