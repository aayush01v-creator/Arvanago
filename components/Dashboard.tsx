...
const StatCard: React.FC<{ icon: string; value: string; label: string; color: string; delay?: number }> = ({ icon, value, label, color, delay = 0 }) => {
    const ref = useScrollAnimation();
    return (
        <div ref={ref} className="bg-white dark:bg-slate-800 rounded-2xl p-6 flex items-center shadow-md dark:shadow-dark-glow border border-slate-100 dark:border-slate-700 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:hover:shadow-dark-glow-hover dark:hover:border-brand-primary/30 scroll-animate" style={{ transitionDelay: `${delay}ms`}}>
...
const DashboardCourseCard: React.FC<{ course: Course; navigateToCourse: (course: Course) => void; }> = ({ course, navigateToCourse }) => {
    const ref = useScrollAnimation();
    return (
        ...
        <div className="p-5 flex flex-col flex-grow">
            ...
            <div className="mt-4 mb-5">
                ...
                <div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2">
                    <div className="bg-brand-primary h-2 rounded-full" style={{ width: `${course.progress}%` }}></div>
                </div>
            </div>
            <button 
...
const CategoryCard: React.FC<{ category: { name: string; icon: string; color: string }; navigateToFilteredCourses: (category: string) => void }> = ({ category, navigateToFilteredCourses }) => {
    const ref = useScrollAnimation();
    return (
    <div 
        ref={ref}
